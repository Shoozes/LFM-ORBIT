...
Mission Evidence wording: target packs and CV boxes are product-facing evidence, not separate operator tools. Active missions carry `target_pack_id` and `object_targets`; Mission Evidence runs the active target pack through batch grounding. The main public showcase pack is `critical_minerals`; `deforestation`, `fireline`, `port`, `glacier`, `waterline`, `lifeline`, `camp`, and cautious `plastic`/coastal-debris packs remain available through presets and backend contracts. Alert proofs can carry persisted `detection_summary` and `object_deltas`; replay snapshots and dataset exports preserve mission target intent for cost control and future training.
