...
- Harden alternate local dev/test ports so demo export can move off default API/Vite ports without CORS or runtime URL issues.
...
- Demo refresh: `npm run demo:record`, `npm run demo:object-evidence`, and `npm run demo:tutorial` from `source/frontend`
...
- Full Playwright passed `101` with `1` intentional HTML-dump skip.
- Docs artifact guard passed `15`.
