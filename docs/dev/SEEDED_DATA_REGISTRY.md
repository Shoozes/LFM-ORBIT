...
Sentinel Hub is a development/cache-seeding source for these real-data fixtures. The default full-app runtime remains SimSat/Mapbox, with these files used as bundled cached replay proof so reviewers can reproduce the same result without Sentinel Hub credentials or quota. Every source-backed `sh_*.webm` and matching metadata file referenced by the replay catalog or summary bank is tracked. Generated `nasa_*` cache outputs remain runtime-only except for the explicitly tracked `nasa_aa01bc81.webm` frontend E2E context-timelapse fixture.
...
- Generated timelapse requests use version-2 keys derived from normalized bbox, effective month samples, and provider preference. A same-bbox request with a different date window, sampling density, or provider must not reuse generated bytes.
- The legacy eight-character bbox-only key remains valid only for explicit committed replay-fixture readers such as seeded promotion. Do not use it as the generated-request cache key.
