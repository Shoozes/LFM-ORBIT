...
The portfolio route is a separate frontend-only contract. The hosted build renders at `/`; the standard Pages build renders at a configured project path such as `/LFM-ORBIT/`; the full local build exposes the same surface at `/hosted`. Model-enabled builds load and validate `source/frontend/hosted/model-manifest.json`, which Vite emits only when the model policy is enabled, then use Wllama/WebAssembly to fetch the pinned `LFM2.5-VL-450M-Q4_0.gguf` revision from [Shoozes/lfm2.5-450m-vl-orbit-satellite](https://huggingface.co/Shoozes/lfm2.5-450m-vl-orbit-satellite) before running short local text chat against saved demo-package context. Pages mode is fail-closed and the committed workflow sets `VITE_HOSTED_MODEL_ENABLED=false`, so its app does not request model metadata or weights and does not ship the model runtime while redistribution terms are unresolved. Public model builds require HTTPS; when cross-origin isolation is unavailable, the loader uses Wllama single-thread mode for safer mobile compatibility. It does not use the backend GGUF path, FastAPI, provider credentials, live imagery, or browser-side image inputs. Do not merge this sealed browser artifact contract with the moving full-app model handoff.
