...
LFM Orbit has two presentations: the full operator app at `/`, backed by FastAPI, and a separate browser-only portfolio route. The normal build exposes the hosted route at `/hosted`; `npm run build:hosted` emits the model-enabled hosted presentation at `/` without the full app, while `npm run build:pages` emits a fail-closed saved-evidence presentation under a configurable Pages project base unless `VITE_HOSTED_MODEL_ENABLED=true` is explicitly supplied. The full frontend is organized around five operator surfaces plus one replay proof surface:
...
10. `core/acquisition.py` produces stable acquisition fingerprints from provider/acquisition/frame identity when present, or from the exact source/cell/temporal evidence windows as a deterministic fallback.
11. `core/paths.py` centralizes repo-root runtime-data/model/boundary paths so cache and model resolution do not drift with working directory.
12. `core/model_manifest.py` resolves the trained satellite artifact from a runtime manifest plus environment overrides.
13. `core/object_targets.py` loads versioned default target packs plus runtime custom packs, normalizes object targets, rejects unsafe labels, and backs the `/api/object-targets/*` registry routes.
14. `core/mission.py` stores mission-level `target_pack_id`, `object_targets`, and a persisted `confirmation_policy`, exposes add/remove/set/clear mutation helpers, and keeps old mission rows migration-safe. New one-shot missions default to `single_acquisition`; recurring monitors can explicitly request `distinct_acquisition`.
15. `core/object_evidence.py` runs enabled mission targets through batch grounding, normalizes boxes to `unit_xyxy`, summarizes counts, and preserves provenance.
16. `core/mission_archive.py` preserves mission intent before runtime resets and feeds dataset exports with target packs/object targets without new provider calls.
17. `core/scene_qc.py` rejects unusable scenes before scoring.
18. `core/scorer.py` and `core/analyzer.py` compute deltas, confidence, and interpretation signals.
19. `core/overlays/attribution.py` adds governance/boundary context when available.
20. `core/telemetry.py` emits typed `scan_result` payloads defined in `core/contracts.py`.
21. `core/queue.py` persists confirmed alerts for `Logs`, `Inspect`, and recent-alert APIs. Alert rows retain mission/use-case/target-pack identity, while anomaly candidates are keyed by mission, cell, and acquisition fingerprint so repeated cached evidence is idempotent and streaks cannot leak across missions.
22. `core/gallery.py` expands confirmed alerts into imagery/timelapse evidence, selects the retained image passed to visual review, and reuses local replay cache imagery for thumbnail fallback before dropping to offline chips; dataset export rasterizes SVG fallbacks to PNG.
23. `core/replay.py` can reset runtime state and load a completed mission directly into the same mission, queue, gallery, metrics, and dialogue stores used by realtime operations. `core/replay_snapshot.py` preserves and validates mission confirmation policy during round-trip restore. Replay also exposes cached API WebMs as Replay Cache entries and can rescan the same retained frames/evidence through the current prompt/model review stack without provider fetches.
24. `ValidationPanel.tsx` and `TimelapseViewer.tsx` expand a selected alert into imagery, analysis, exports, and timelapse context. Mission-tab timelapse output stays separate from mission setup so active temporal video evidence remains visible.
...
26. `GroundAgent.tsx`, `/api/agent/chat`, and `/api/agent/action/confirm` provide a local action chat that proposes and confirms replay loads/rescans, mission packs, SAT/GND link changes, and map-navigation actions before mutating app state.
27. `ProofModePanel.tsx` turns replay state into visible Proof Mode output: satellite frame, bbox/evidence overlay, evidence result, image-conditioned retained-frame review status when enabled, latency, provenance, raw-vs-alert bytes, reduction ratio, abstain state, backend-derived link outage queue, and proof JSON. Its async hydration is keyed to mission identity and snapshots high-churn telemetry inputs so live mission refreshes do not cancel an in-flight related-timelapse request.
28. `source/frontend/hosted/HostedEvidenceDemo.tsx` is the model-free presentation surface with a validated `source/frontend/public/demo-packages/index.json` manifest; `source/frontend/hosted/HostedDemo.tsx` is the model-enabled wrapper used by the local `/hosted` route and `build:hosted`, while `hosted-main.tsx` is the Pages-safe entry and `hosted-model-main.tsx` is selected only for model-enabled hosted builds. Reviewed stills live under `source/frontend/public/demo-assets/`. `hostedConfig.ts` owns the project-base-aware asset resolver and parsed deployment capability, `hostedModel.ts` validates the build-emitted `source/frontend/hosted/model-manifest.json`, and `useBrowserModel.ts` exposes loading/ready/error manifest states, verifies the pinned HTTPS pointer/byte inventory only after an explicit enabled-build fetch, and keeps download cancellation separate from reusable-instance generation cancellation before Wllama/WebAssembly loads it. `TerrainShaderCanvas.tsx` supplies the HAIO-inspired landing visual and pauses when motion/visibility conditions require it.
...
- `source/frontend/hosted/HostedEvidenceDemo.tsx`, `source/frontend/hosted/HostedDemo.tsx`, `source/frontend/hosted/hosted.css`
  Separate portfolio presentation for saved evidence, optional browser model fetch, local chat, and short edge-AI lessons. The model-free entry makes saved evidence the primary action and omits model/chat controls; both entries deliberately have no backend/provider/API controls.
...
- Async refresh guard: the shared frontend request gate aborts superseded requests and ignores stale responses; mission polling and Agent Dialogue bus stats also abort on close/unmount, telemetry JSON refreshes have a five-second ceiling, and agent-dialogue WebSocket payloads are validated with bounded retention and capped reconnect backoff.
...
- The hosted build is a static/read-only presentation with schema-v2 saved packages. Each package traces to a local replay manifest and keeps runtime truth, imagery origin, scoring basis, observation window, bbox, and retention decision visible. The Pages entry is model-free by default; model-enabled builds are selected explicitly, emit the manifest through the Vite build boundary, require HTTPS, and use single-thread Wllama loading when cross-origin isolation is unavailable. Inference is device-dependent and the first model fetch is intentionally user-triggered; every route must not imply live imagery or backend/API availability. The capability probe keeps insecure, unsupported, or resource-constrained browsers on saved packages.
