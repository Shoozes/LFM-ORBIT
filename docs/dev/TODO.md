...
- May 6 retrained model refresh #2: cold reset passed, the latest `Shoozes/lfm2.5-450m-vl-orbit-satellite@main` was forced from remote revision `40a2ebdfeab056a7e0da8c41677d592e8bd2199d`, local training metadata reports task `orbit-satellite-triage`, `7245` train/multimodal rows, `8335` image blocks, and `805` eval rows, and GGUF SHA256 is `7A0A6535BB144487AEBFFD947C56BEDCDC15A1F7F72B9E0D83A63308FB37B4E3`. A concurrent llama.cpp crash found during tutorial Playwright was fixed by serializing shared GGUF completions; `.\run.ps1 -Verify` passed with backend `465 passed`, GGUF smoke, frontend lint/build, and Playwright passing.
