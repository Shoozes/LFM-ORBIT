...
- `.\run.ps1 -Verify` passed end to end on May 5, 2026: backend `453 passed`, trained GGUF smoke passed, frontend lint/build passed, and Playwright passed `94` with `6` intentional skips for HTML dump and retired visual-overlay UI specs.
...
- May 5 Area Tools media refresh: `npm run demo:record` passed `5`, `npm run demo:tutorial` passed `1`, `npm run test:e2e` passed `94` with `6` intentional skips, frontend `npm run lint` passed, and `uv run --no-sync pytest tests/test_docs_artifacts.py -q` passed `17`.
- May 5 startup/selection check: clean option-1 launcher startup reached backend health and frontend ready with no cold-start mission; Playwright dependencies resolved cleanly from `node_modules`; map-side bbox drawing now has a dedicated draw hitbox and `npx playwright test e2e/bbox.spec.ts` passed `6`, including a real drag-to-grid assertion. Visual confirmation: `output/playwright/selection-tool-drawn-grid.png`.
