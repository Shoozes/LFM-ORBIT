...
- Pages delivery is now reproducible and fail-closed. Done when: the workflow installs Chromium before browser checks, builds a saved-packages-only artifact with an explicit model gate, deploys only `dist-pages`, and runs a post-deploy static-origin smoke with retained reports.
- Hosted model readiness is explicit and cache proof is measurable. Done when: fetch controls require capability plus validated manifest identity, disabled builds never call the model manifest, and the opt-in live harness records transfer bytes and browser-cache provenance across two runs.
- Mission candidate confirmation is acquisition-aware. Done when: candidates are keyed by mission, cell, and stable evidence identity; duplicate fingerprints are idempotent; a new acquisition can confirm a recurring monitor; and queue/scanner/mission tests cover migration and defaults.
...
- August 4 live-origin harness: implemented but not run against a public deployment because the Pages workflow is not yet deployed from the current local changes; it now fails closed without `HOSTED_PAGES_URL`, retains listeners safely, and records model transfer bytes plus disk/service-worker/prefetch cache provenance across two model/chat passes when supplied.
