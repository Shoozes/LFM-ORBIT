...
LFM-ORBIT is a local-first satellite timelapse triage system. A space-side agent scans selected map areas tile-by-tile across a chosen time frame and prunes low-value frames before downlink; a ground-side agent reviews retained evidence packets with provenance, timelapse context, CV boxes, local model reasoning, optional image-conditioned retained-frame review, and compact proof JSON.
...
Current runtime: SimSat/Mapbox hackathon satellite-data API family, default SimSat Sentinel scanning lane, optional SimSat Mapbox imagery/context lane, deterministic replay fixtures for repeatable demos, and shared Satellite/Ground Liquid evidence-packet reasoning when the manifest-resolved local GGUF runtime is available. LiquidAI Leap Tune-compatible training proof is surfaced from `training_result_manifest.json`. Selected retained frames can use the opt-in `/api/inference/image` LiquidAI/LFM2.5-VL-450M image-text-to-text review path when `ORBIT_IMAGE_CONDITIONED_INFERENCE=true` and the `transformers_vlm` adapter passes pixels into the runtime; otherwise Orbit remains in evidence-packet reasoning mode and returns structured unavailable or abstain responses.
...
Optional retained-frame image review uses LiquidAI/LFM2.5-VL-450M through the backend `vision` extra. That extra installs the required Transformers/Torch/Torchvision stack while keeping the base app lightweight:
...
The status APIs report `image_conditioned_runtime_enabled=true` only after a real image adapter call succeeds. The opt-in smoke command is:
...
Configured live provider access can be checked without fallback fixtures:
...
LiquidAI Leap Tune can support the tuning workflow for exported Orbit evidence. Leap Tune remains Liquid AI-owned/open-source tooling.
...
| [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) | LiquidAI Leap Tune and third-party tooling attribution |
...
Operator prompts become product feedback: semantics rows, backend contracts, browser flows, docs, TODO entries, summary-bank groups, and model-training candidates. The repeatable method lives in [docs/dev/AGENT_GROWTH_LOOP.md](docs/dev/AGENT_GROWTH_LOOP.md).
