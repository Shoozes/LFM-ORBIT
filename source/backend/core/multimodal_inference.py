...
_DEFAULT_TRANSFORMERS_TASK = "visual-question-answering"
_DEFAULT_TRANSFORMERS_MODEL = "dandelin/vilt-b32-finetuned-vqa"
...
def _runtime_reason(*, enabled: bool, backend: str, require_mmproj: bool, mmproj_present: bool) -> str:
    ...
    if _IMAGE_REVIEW_PIPELINE is _PIPELINE_UNAVAILABLE:
        return "transformers_vlm image adapter unavailable"
    if _IMAGE_REVIEW_PIPELINE is None:
...
def _get_transformers_review_pipeline():
    ...
    with _IMAGE_RUNTIME_LOCK:
        ...
        try:
            _IMAGE_REVIEW_PIPELINE = pipeline(task, model=model)
...
def _run_transformers_review(*, prompt: str, image: Any, max_tokens: int) -> str:
    ...
    with _IMAGE_RUNTIME_LOCK:
        if task == "visual-question-answering":
            try:
                result = pipe(image=image, question=prompt, top_k=1)
            except TypeError:
            ...
            try:
                result = pipe(images=image, text=prompt, max_new_tokens=max_tokens)
            except TypeError:
                try:
                    result = pipe(image=image, text=prompt, max_new_tokens=max_tokens)
                except TypeError:
...
def multimodal_status(artifact: SatelliteModelArtifact | None = None) -> dict[str, Any]:
    ...
    image_runtime_enabled = adapter_loaded
